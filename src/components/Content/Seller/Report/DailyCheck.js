...
class DailyCheck extends Component {
  ...
  addRow = () => {
    ...
    this.setState((state) => {
      ...
      // });
      if (this.props.material.materials.length === this.state.options.length)
        return;
      else options = [];
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      prevProps.storagereport.storagereports !==
      this.props.storagereport.storagereports
    ) {
      //khi luu cai storage report cuoi cung thi moi thong bao
      if (
        storagereport.storagereports[0].idMaterial ===
      ...
      rows.map((el) => {
        if (el._id === index + 1) {
          const newItem = {
            ...
            quantity: 0,
            quantitydb: selectedMaterial.quantityIndb,
            usedqty: 0,
                              ...
                              <tbody>
                                {this.state.rows.map((el, index) => (
                                  <tr>
                                    ...
                                    <td
                                      onBlur={(e) => this.handleClick(e, index)}
                                      id="used-qty"
                                      bgcolor="#FFFFFF"
                                      style={inputField}
